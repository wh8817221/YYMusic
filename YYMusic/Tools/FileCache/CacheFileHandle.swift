...
class CacheFileHandle: NSObject {
     ...
     */
    static func cacheFileExists(with url: URL) -> String? {
        if let fileName = String.fileName(with: url) {
            let cacheFilePath = "\(String.cacheFolderPath())/\(fileName)"
